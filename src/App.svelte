<script>
	...
	const initalPoints = points.map((point) => ({ ...point, y: 0}));
	const springPoints = spring(initalPoints, {
		stiffness: 0.1,
		damping: 0.9,
		precision: 0.1
	})
	...
	setTimeout(() => {
		springPoints.set(data1)
	}, 100)


	setTimeout(() => {
		springPoints.set(data2)
	}, 1000)
	...
	$: path = `M${$springPoints.map(p => `${xScale(p.x)},${yScale(p.y)}`).join('L')}`;
